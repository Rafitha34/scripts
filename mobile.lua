    ...
    while true do
        ...
        if jobID and jobID ~= ultimoJobId then
            ...
            if setJobIdInput(jobID) then
                task.wait(0.005)
                if clicarBotaoJoin() then
                    prints("🚀 Botão clicado com sucesso!")
                else
                    prints("⚠️ Falha ao clicar no botão 'Join'. Pulando para o próximo JobID.")
                end
            else
                prints("⚠️ Falha ao preencher o campo de texto. Pulando para o próximo JobID.")
            end
            ultimoJobId = jobID
        end
