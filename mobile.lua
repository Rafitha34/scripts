...
task.spawn(function()
    ...
    while true do
        ...
        if jobID and jobID ~= ultimoJobId then
            ...
            -- Tenta preencher o campo de texto
            local inputSuccess = setJobIdInput(jobID)
            if inputSuccess then
                task.wait(0.005) -- Aguarda 0.005 segundos
                
                -- Tenta clicar no botão
                local clickSuccess = clicarBotaoJoin()
                if clickSuccess then
                    prints("🚀 Botão clicado com sucesso!")
                    ultimoJobId = jobID -- Atualiza o último JobID somente após sucesso
                else
                    prints("⚠️ Falha ao clicar no botão 'Join'. Tentando novamente no próximo loop.")
                end
            else
                prints("⚠️ Falha ao preencher o campo de texto. Tentando novamente no próximo loop.")
            end
        end
