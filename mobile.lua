...
-- NOVA LÓGICA DE PREENCHIMENTO E BOTÃO
...
local function delayAleatorio()
    return math.random(20, 80) / 1000
end

-- Execução principal em loop usando a nova lógica
task.spawn(function()
    local ultimoJobId = nil
    local ignorarPrimeiro = true

    -- Verifica a chave apenas uma vez antes de iniciar o loop
    if not checkKey() then
        prints("❌ A chave é inválida ou não autorizada. Encerrando o script.")
        return
    end

    print("✅ Chave validada com sucesso! Iniciando monitoramento de Job IDs...")
    print("⏳ Esperando novos job ids...")

    while true do
        local jobID = readJobID()
        if jobID and jobID ~= ultimoJobId then
            if ignorarPrimeiro then
                ignorarPrimeiro = false
            else
                print("🆕 Novo job id encontrado: " .. tostring(jobID) .. "^")
                if setJobIdInput(jobID) then
                    task.wait(delayAleatorio())
                    print("🚀 Teleporte tentado com sucesso!")
                    clicarBotaoJoin()
                end
            end
            ultimoJobId = jobID
