...
-- Execução principal (SEM FILA / múltiplas sequências paralelas)
local activeOrDoneJobIDs = {}
local lastSeenJobID = nil

local function startSequence(jobID)
    if activeOrDoneJobIDs[jobID] then
        return -- Já em execução ou finalizado
    end
    activeOrDoneJobIDs[jobID] = true
    task.spawn(function()
        for i = 1, 3 do
        ...
        prints("✅ Sequência finalizada para JobID: " .. jobID)
    end)
end
...
task.spawn(function()
    ...
    while true do
        ...
        if jobID then
            ...
            -- Inicia sequência somente se ainda não iniciada para esse ID
            startSequence(jobID)
        end
        task.wait(0.5) -- Checagem a cada 0.5s
    end
end)
