...
local TARGET_COORDINATES = {
    ...
    Vector3.new(-479, 19.1, -99.9),
    Vector3.new(-340, 19.1, -100),
    Vector3.new(-340, 19.1, 5.5),
...
local function handleDesync()
    ...
    if isDesyncActive then
        print("[Desync] Ativando...")
        if not enableMobileDesync() then
            isDesyncActive = false -- Falhou, reverte o estado
            print("[Desync] Falha ao ativar.")
        else
            print("[Desync] Ativado.")
        end
    else
        disableMobileDesync()
        print("[Desync] Desativado.")
    end
end
